package org.learning.communication.assignment.multipartformdata;
...
public class Assignment_6_Stub_For_File_Upload_XML {
    ...
    @Test
    @DisplayName("Verify the file upload using multipart/form-data type for xml")
    public void testMultipartFormDataXml() throws Exception {
        var fileName = "Request_body_for_pet_in_xml-list.txt";
        File file = readerUtils.readeFile(fileName);

        var stub = WireMock.post(WireMock.urlPathEqualTo("/pet/upload"))
                .withQueryParam("format", WireMock.equalTo("XML"))
                .withMultipartRequestBody(
                        WireMock.aMultipart()
                                .withName("file")
                                .withFileName(fileName)
                                .withBody(WireMock.matchingXPath("//pets/pet[id=1]/id", WireMock.equalToXml("<id>1</id>")))
        ...
        try {
            ...
            var response = communication.uploadDataUsingFile(file, "XML");
